...
const Page = ({ params }: { params: Promise<{ budgetId: string }> }) => {
  ...
  const handleAddTransaction = async () => {
    ...
    try {
      ...
      setNotification("Nouvelle transaction ajoutée avec succès !");
      setAmount("");
      setDescription("");
  ...
  const handleDeleteBudget = async () => {
    ...
    if (confirmed) {
      try {
        ...
        setNotification(`Erreur lors de la suppression du budget : ${error}`);
      }
      redirect("/budgets");
    }
  };
  ...
  const handleDeleteTransaction = async (transactionId: string) => {
    ...
    if (confirmed) {
      try {
        ...
        fetchBudgetData(budgetId);
        setNotification("Dépense supprimée avec succès !");
      } catch (error) {
        setNotification(
          `Erreur lors de la suppression de la dépense : ${error}`
        );
      }
    }
  };
      ...
      {budget && (
        <div className="flex md:flex-row flex-col">
          ...
          {budget?.transactions && budget.transactions.length > 0 ? (
            ...
            <div className="md:w-3/3 mt-10 md:ml-4 flex items-center justify-center">
              <Send strokeWidth={1.5} className="w-8 h-8 text-accent" />
              <span className="text-gray-500 ml-2">
                Aucune transaction pour le moment. Ajoutez-en une !
              </span>
            </div>
          )}
        </div>
